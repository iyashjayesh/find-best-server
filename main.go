	...
	for _, symbol := range symbols {
		if strings.Contains(text, symbol) {
			if strings.HasPrefix(text, symbol) {
				priceText := strings.Split(text, " ")[0]
...
func FetchGoogleLinksAPI(query string, country string) []string {
	...
	apiKey := os.Getenv("GOOGLE_API_KEY_ENV")
	cseID := os.Getenv("GOOGLE_CSE_ID_ENV")
...
// ScrapeProductPage fetches the product page and extracts the title and price
// It uses multiple selectors to ensure we can find the price in different formats
